...
def mock_cancel_task_run_asyncio(mocker):
    return mocker.patch(
        "render.public_api.api.workflow_tasks_beta.cancel_task_run.asyncio_detailed"
    )
...
def mock_list_task_runs_asyncio(mocker):
    return mocker.patch(
        "render.public_api.api.workflow_tasks_beta.list_task_runs.asyncio_detailed"
    )
...
def mock_create_task_asyncio(mocker):
    return mocker.patch(
        "render.public_api.api.workflow_tasks_beta.create_task.asyncio_detailed"
    )
...
def mock_get_task_run_asyncio(mocker):
    return mocker.patch(
        "render.public_api.api.workflow_tasks_beta.get_task_run.asyncio_detailed"
    )
...
def mock_httpx_stream(mocker):
    ...
    mocker.patch("render.client.workflows.httpx.AsyncClient", return_value=client_cm)
