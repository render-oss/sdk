...
The worker path (``from render import Workflows``) deliberately avoids
...
If you find yourself adding an eager top-level ``from render.<heavy>
import ...`` to a module on the worker path, lazy-load it instead — see
``render/__init__.py`` and ``render/client/__init__.py`` for the
...
# Modules that must NOT be loaded by `from render import Workflows`.
...
def _modules_loaded_by(import_statement: str) -> set[str]:
    ...
    result = subprocess.run(  # noqa: S603 — code is a constructed literal, not user input
...
def test_worker_path_does_not_load_rest_or_experimental_modules() -> None:
    loaded = _modules_loaded_by("from render import Workflows")
    ...
    assert not leaked, (
        "Importing `render.Workflows` eagerly loaded modules that are "
