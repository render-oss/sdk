...
from render.client.errors import ClientError, ServerError
from render.experimental.key_value.api import KeyValueApi
from render.experimental.key_value.types import KeyValueInstance
from render.public_api.models.database_status import DatabaseStatus
from render.public_api.models.key_value import KeyValue
from render.public_api.models.key_value_connection_info import (
    KeyValueConnectionInfo,
)
from render.public_api.models.key_value_detail import KeyValueDetail
from render.public_api.models.key_value_options import KeyValueOptions
from render.public_api.models.key_value_patch_input import KeyValuePATCHInput
from render.public_api.models.key_value_plan import KeyValuePlan
from render.public_api.models.key_value_post_input import KeyValuePOSTInput
from render.public_api.models.key_value_with_cursor import KeyValueWithCursor
from render.public_api.models.owner import Owner
from render.public_api.models.owner_type import OwnerType
from render.public_api.models.region import Region
...
class TestFindById:
    ...
    async def test_handles_api_token_error(self, api, mocker):
        mocker.patch(
            "render.experimental.key_value.api.retrieve_key_value.asyncio_detailed",
...
class TestFindByName:
    ...
    async def test_handles_api_token_error(self, api, mocker):
        mocker.patch(
            "render.experimental.key_value.api.list_key_value.asyncio_detailed",
            new_callable=AsyncMock,
            return_value=err(401),
        )
        with pytest.raises(ClientError, match="API Token is not authorized"):
...
class TestGetConnectionInfo:
    ...
    async def test_handles_api_token_error(self, api, mocker):
        mocker.patch(
            "render.experimental.key_value.api.retrieve_key_value_connection_info.asyncio_detailed",
            new_callable=AsyncMock,
            return_value=err(401),
        )
        with pytest.raises(ClientError, match="API Token is not authorized"):
...
class TestCreateInstance:
    ...
    async def test_handles_api_token_error(self, api, mocker):
        mocker.patch(
            "render.experimental.key_value.api.create_key_value.asyncio_detailed",
            new_callable=AsyncMock,
            return_value=err(401),
        )
...
class TestUpdateInstance:
    ...
    async def test_handles_api_token_error(self, api, mocker):
        mocker.patch(
            "render.experimental.key_value.api.update_key_value.asyncio_detailed",
            new_callable=AsyncMock,
            return_value=err(401),
        )
        with pytest.raises(ClientError, match="API Token is not authorized"):
